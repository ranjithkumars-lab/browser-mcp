...
class TransferToolkit:
    ...
    @tool(
        ...
        description="Capture and save the next browser download.",
        returns="json",
    )
    async def download(
        self,
    ...
    @tool(
        name="browser.upload",
        description="Upload local files through an input, chooser, or drag-and-drop target.",
        returns="json",
    )
    async def upload(
        self,
    ...
    @tool(
        name="browser.transfer.status",
        description="Get transfer lifecycle state and progress.",
        returns="json",
    )
    async def status(self, transfer_id: str) -> dict[str, Any]:
        try:
            return (await self._manager.status(transfer_id)).to_dict()
        except Exception as exc:
            return {"success": False, "error": str(exc), "transfer_id": transfer_id}
    ...
    async def cancel(self, transfer_id: str) -> dict[str, Any]:
        try:
            return (await self._manager.cancel(transfer_id)).to_dict()
        except Exception as exc:
            return {"success": False, "error": str(exc), "transfer_id": transfer_id}
