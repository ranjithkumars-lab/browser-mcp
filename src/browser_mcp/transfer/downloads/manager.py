...
class DownloadManager:
    def __init__(
        ...
        *,
        ...
        checksum_algorithm: ChecksumAlgorithm | str,
    ) -> None:
        self._registry, self._naming, self._verifier = registry, naming, verifier
        self._directory, self._collision_strategy, self._checksum_algorithm = (
            directory,
            collision_strategy,
            checksum_algorithm,
        )
    ...
    async def download(
        ...
        *,
        ...
        destination = self._naming.destination(
            self._directory, file_name or "download", collision_strategy=self._collision_strategy
        )
        info = await self._registry.get(strategy).execute(
            page,
        ...
        if file_name is None and info["file_name"] != destination.name:
            final = self._naming.destination(
                self._directory, str(info["file_name"]), collision_strategy=self._collision_strategy
            )
            saved.replace(final)
        ...
        info["checksum"] = self._verifier.verify(
            saved, algorithm=self._checksum_algorithm, expected=expected_checksum
        )
        return info
