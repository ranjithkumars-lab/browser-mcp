...
from browser_mcp.transfer.errors import FileNotFoundError, FileSizeExceededError, InvalidMimeTypeError


class FileValidator:
    def __init__(self, *, max_file_size_bytes: int, allowed_extensions: list[str] | None = None, allowed_mime_types: list[str] | None = None) -> None:
    ...
    def validate(self, files: list[str]) -> list[dict[str, Any]]:
        ...
        for value in files:
            ...
            if size > self.max_file_size_bytes:
                raise FileSizeExceededError(f"upload file '{path.name}' exceeds {self.max_file_size_bytes} bytes")
            ...
            result.append({"file_name": path.name, "file_path": str(path.resolve()), "file_size_bytes": size, "mime_type": mime})
        return result
