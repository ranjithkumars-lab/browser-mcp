...
class UploadManager:
    ...
    async def upload(self, page: Any, *, selector: str, files: list[str], strategy: str = "input", frame_id: str | None = None) -> list[dict[str, Any]]:
        metadata = self._validator.validate(files)
        await self._registry.get(strategy).execute(page, selector=selector, files=[str(row["file_path"]) for row in metadata], frame_id=frame_id)
        return metadata
