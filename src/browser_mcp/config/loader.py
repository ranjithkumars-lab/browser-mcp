"""Hierarchical configuration loader for the Browser MCP server.
...
from browser_mcp.config.defaults import DEFAULT_ENV, ENV_VAR_PREFIX
from browser_mcp.config.models import BrowserSettings
from browser_mcp.config.paths import bundled_settings_dir
from browser_mcp.errors import ProfileError

__all__ = ["load_browser_settings"]
...
def _read_yaml(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise ProfileError(f"failed to parse YAML file '{path}': {exc}") from exc
    if raw is None:
        return {}
    if not isinstance(raw, dict):
        raise ProfileError(f"YAML file '{path}' must contain a mapping at its root")
...
def _env_overrides() -> dict[str, Any]:
    """Return values explicitly provided through environment variables."""
...
def load_browser_settings(
    *,
    ...
    """Load and validate browser settings.

    Parameters
    ----------
    env:
        Environment name. Falls back to ``BROWSER_MCP_ENV`` then ``development``.
    settings_dir:
        Directory containing ``default.yaml`` and ``{env}.yaml``. Defaults to the
        bundled browser settings directory.
    overrides:
    ...
    return BrowserSettings.model_validate(merged)
