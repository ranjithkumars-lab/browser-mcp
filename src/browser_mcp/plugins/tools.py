...
class PluginToolkit:
    ...
    async def list_plugins(self) -> dict[str, Any]:
        return {"success": True, "plugins": self._manager.list()}
    ...
    async def info(self, name: str) -> dict[str, Any]:
        try:
            ...
            return {
                "success": True,
                "plugin": value.model_dump(mode="json")
    ...
    async def execute(self, name: str, payload: dict[str, Any]) -> dict[str, Any]:
        try:
            return {"success": True, "result": await self._manager.execute(name, payload)}
        except Exception as exc:
            return {"success": False, "error": str(exc)}
    ...
    def register(self, registry: Any) -> None:
        for name in ("list_plugins", "info", "execute", "reload"):
