...
class PluginRuntime:
    def __init__(self, manifest: PluginManifestV2, plugin: Any, context: Any, *, timeout_seconds: float = 30.0) -> None:
    ...
    async def activate(self) -> None:
        await self.plugin.initialize(self.context); self.state = PluginState.ACTIVATED

    async def execute(self, payload: dict[str, Any]) -> Any:
        self._schemas.validate(payload, self.manifest.inputs)
        result = await self._executor.execute(self.plugin, payload, timeout_seconds=self.timeout_seconds)
        self._schemas.validate(result, self.manifest.outputs)
        return result

    async def deactivate(self) -> None:
        await self.plugin.shutdown(); self.state = PluginState.DEACTIVATED
