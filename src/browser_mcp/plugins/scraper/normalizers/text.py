...
class TextNormalizer:
    ...
    def normalize(self, raw: dict[str, Any], meta: ScrapeMeta) -> TextResult:
        ...
        if "texts" in raw:
            ...
            texts: list[str] = [str(x) for x in cast(list[object], raw_texts)] if isinstance(raw_texts, list) else []
            joined = " ".join(texts).strip()
            words = joined.split() if joined else []
            raw_sel: object = raw.get("selectors_only")
            selectors_only: list[str] = [str(x) for x in cast(list[object], raw_sel)] if isinstance(raw_sel, list) else []
            return TextResult(
                meta=meta,
                text=joined,
