...
class BrowserEventManager:
    ...
    async def publish_domain_event(self, event: DomainEvent) -> None:
        ...
        priority = (
            EventPriority.HIGH
            if event.event_name.endswith((".failed", ".error"))
            else EventPriority.NORMAL
        )
        await self.publish(
            BrowserEvent(
                header=EventHeader(
                    event_id=event.event_id, timestamp=event.occurred_at, priority=priority
                ),
    ...
    def listen(self, pattern: str, handler: Subscriber) -> None:
        self._router.subscribe(pattern, handler)

    def query(self, **filters: Any) -> list[BrowserEvent]:
        return self._store.query(**filters)

    def replay(self, event_id: str | None = None) -> list[BrowserEvent]:
        return self._store.replay(event_id)
