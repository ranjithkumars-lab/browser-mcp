...
class RedisBrokerProvider(BrokerProvider):
    ...
    def __init__(self) -> None:
        self._queues = {k: asyncio.Queue[dict[str, Any]]() for k in ("high", "default", "low")}
        self.dlq: list[dict[str, Any]] = []

    async def enqueue(self, payload: dict[str, Any], priority: str = "default") -> None:
        await self._queues.get(priority, self._queues["default"]).put(payload)

    async def dequeue(self) -> dict[str, Any] | None:
        for name in ("high", "default", "low"):
            if not self._queues[name].empty():
